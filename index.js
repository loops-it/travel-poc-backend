...
app.post('/extract-details', async (req, res) => {
  ...
  try {
    const messages = [
      ...
      { 
        role: 'user', 
        content: `Please analyze well and extract all the customer details (name, phone number, email, expected destination, expected date, budget, and expected return date) from the following text. If only the number of days for the trip is given, calculate the expected return date: \n\n${text}` 
      }
    ];
    ...
    const extractedDetails = completion.choices[0].message.content;
